...
const DEFAULT_RULES = [
  ...
  { name: 'cookie', type: 'property', message: '敏感属性赋值 (cookie)' },
  { name: 'domain', type: 'property', message: '敏感属性赋值 (domain)' },

  // --- 函数调用类 (Call/New Expression) ---
  
  // 1. 动态执行与前端 XSS 节点
  { name: 'eval', type: 'call', message: '动态代码执行 (eval)' },
  { name: 'Function', type: 'call', message: '动态函数创建 (new Function)' },
  { name: 'setTimeout', type: 'call', message: '潜在动态执行 (setTimeout)' },
  { name: 'setInterval', type: 'call', message: '潜在动态执行 (setInterval)' },
  { name: 'write', type: 'call', message: '直接文档流输出 (document.write)' },
  { name: 'writeln', type: 'call', message: '直接文档流输出 (document.writeln)' },
  { name: 'insertAdjacentHTML', type: 'call', message: 'JS HTML注入操作 (insertAdjacentHTML)' },

  // 2. 服务端命令注入与子进程调用
  { name: 'exec', type: 'call', message: '服务端命令注入 (exec)' },
  { name: 'execSync', type: 'call', message: '服务端同步命令注入 (execSync)' },
  { name: 'spawn', type: 'call', message: '服务端命令注入 (spawn)' },
  { name: 'spawnSync', type: 'call', message: '服务端同步命令注入 (spawnSync)' },
  { name: 'fork', type: 'call', message: '服务端进程创建 (fork)' },
  { name: 'execFile', type: 'call', message: '服务端核心调用 (execFile)' },
  { name: 'execFileSync', type: 'call', message: '服务端同步核心调用 (execFileSync)' },

  // 3. 服务端沙箱代码执行 (vm 模块)
  { name: 'runInContext', type: 'call', message: '虚拟机代码执行 (runInContext)' },
  { name: 'runInNewContext', type: 'call', message: '虚拟机代码执行 (runInNewContext)' },
  ...
  { name: 'writeFileSync', type: 'call', message: '同步文件系统写入 (writeFileSync)' },
  { name: 'unlink', type: 'call', message: '文件系统删除 (unlink)' },
  { name: 'unlinkSync', type: 'call', message: '同步文件系统删除 (unlinkSync)' },

...
const queries = {
  javascript: {
    injections: safeLoadQuery(JavaScript, 'injections-js.scm'),
    xss: safeLoadQuery(JavaScript, 'xss-js.scm'),
    secrets: safeLoadQuery(JavaScript, 'secrets-js.scm')
  },
  typescript: {
    injections: safeLoadQuery(TypeScript, 'injections-js.scm'),
    xss: safeLoadQuery(TypeScript, 'xss-js.scm'),
    secrets: safeLoadQuery(TypeScript, 'secrets-js.scm')
  },
  tsx: {
    injections: safeLoadQuery(TSX, 'injections-js.scm'),
    xss: safeLoadQuery(TSX, 'xss-js.scm'),
    react: safeLoadQuery(TSX, 'react.scm'),
    secrets: safeLoadQuery(TSX, 'secrets-js.scm')
  },
  vue: {
    xss: safeLoadQuery(Vue, 'html.scm')
  },
  html: {
    xss: safeLoadQuery(HTML, 'html.scm')
  }
};
  ...
  try {
    if (ext === '.vue') {
      ...
      if (descriptor.template) {
        ...
        // Robust query selection: if Vue-specific query failed, use HTML query
        let templateQueries = config.queries;
        if (!templateQueries || (!templateQueries.xss && queries.html)) {
          templateQueries = queries.html;
        }

        const tree = config.parser.parse(templateContent);
        const templateSinks = runQueries(tree, templateContent, templateQueries);
